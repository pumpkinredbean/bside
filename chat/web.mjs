...
import { createDecider, structuredReply } from "./core.mjs";
...
<h1><b>chat</b>jev</h1>
<div class="sub">no text generation — plan → fill → verify → repair, every step a typed Jev decision · ${VOCAB.length}-word vocab</div>
<div id="log"></div>
...
async function send() {
  ...
  let t0 = Date.now();
  const es = new EventSource("/api/chat?msg=" + encodeURIComponent(msg));
  es.onmessage = (e) => {
    ...
    if (ev.e === "plan") {
      ...
      stage(ev.ok >= 0.5 ? "verify ✓ grammatical (" + ev.ok.toFixed(2) + ")" : "verify ✗ score " + ev.ok.toFixed(2) + " — repairing…");
    } else if (ev.e === "repair") {
      ...
      bot.insertAdjacentHTML("beforeend",
        '<div class="meta">' + ev.words + " words · " + dt.toFixed(1) + "s · " + (dt/Math.max(1,ev.words)).toFixed(2) + "s/word" +
        " · " + ev.inTok.toLocaleString() + " in / " + ev.outTok.toLocaleString() + " out tok · est $" + (ev.inTok*0.042/1e6).toFixed(4) +
        " · hover a word for slot + probability</div>");
...
const server = createServer(async (req, res) => {
  ...
  if (u.pathname === "/api/chat") {
    ...
    try {
      await structuredReply(d, msg, { onEvent: send });
    } catch (e) {
